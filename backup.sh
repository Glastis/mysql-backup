...
while getopts "hfasu:p:d:o:" arg; do
    case ${arg} in
        h)
            usage
            ;;
...
{
    if [[ -n ${MYSQL_USERNAME} ]];              then MYSQL_COMMAND="${MYSQL_COMMAND} -u ${MYSQL_USERNAME}"; fi
    if [[ -n ${MYSQL_PASSWORD} ]];              then MYSQL_COMMAND="${MYSQL_COMMAND} -p${MYSQL_PASSWORD}"; fi

    if [[ -n ${MYSQL_USERNAME} ]];              then MYSQLDUMP_COMMAND="${MYSQLDUMP_COMMAND} -u ${MYSQL_USERNAME}"; fi
    if [[ -n ${MYSQL_PASSWORD} ]];              then MYSQLDUMP_COMMAND="${MYSQLDUMP_COMMAND} -p${MYSQL_PASSWORD}"; fi
}
